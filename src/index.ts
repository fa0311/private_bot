...
const env = await getEnv(
  z.object({
    ...
    TWITTER_COOKIE_FILE: z.string().default("cookie.json"),

    DISCORD_BOT_TOKEN: z.string(),
    DISCORD_PUSH_TOKEN: z.string(),
    DISCORD_PUSH_BASE_URL: z.string().default("https://discordapp.com/api/webhooks"),
    DISCORD_SYNCHRONIZE_CHAT_CHANNEL_ID: z.string().optional(),
    DISCORD_SYNCHRONIZE_CHAT_GUILD_ID: z.string().optional(),
    DISCORD_SYNCHRONIZE_VOICE_CHANNEL_ID: z.string().optional(),
    DISCORD_SYNCHRONIZE_VOICE_GUILD_ID: z.string().optional(),
    DISCORD_SET_PRESENCE_ACTIVITIES_NAME: z.string().default("bot"),
    DISCORD_SET_PRESENCE_ACTIVITIES_URL: z.string().default("https://example.com"),

    LINE_SYNCHRONIZE_CHAT_CHANNEL_ID: z.string(),
  }),
);
...
const discordPush = createDiscordWebHookClient({
  token: env.DISCORD_PUSH_TOKEN,
  baseUrl: env.DISCORD_PUSH_BASE_URL,
});
...
const twitterClient = await createTwitterClient(env.TWITTER_COOKIE_FILE);
...
discordClient.client.user?.setPresence({
  ...
  activities: [
    {
      name: env.DISCORD_SET_PRESENCE_ACTIVITIES_NAME,
      url: env.DISCORD_SET_PRESENCE_ACTIVITIES_URL,
    },
  ],
});
...
discordClient.client.on(Events.MessageCreate, async (message) => {
  ...
  if (env.DISCORD_SYNCHRONIZE_CHAT_GUILD_ID && env.DISCORD_SYNCHRONIZE_CHAT_GUILD_ID !== message.guildId) return;
  if (env.DISCORD_SYNCHRONIZE_CHAT_CHANNEL_ID && env.DISCORD_SYNCHRONIZE_CHAT_CHANNEL_ID !== message.channelId) return;
...
discordClient.client.on(Events.VoiceStateUpdate, async (before, after) => {
  ...
  if (env.DISCORD_SYNCHRONIZE_VOICE_GUILD_ID && env.DISCORD_SYNCHRONIZE_VOICE_GUILD_ID !== guild.id) return;
  if (env.DISCORD_SYNCHRONIZE_VOICE_CHANNEL_ID && env.DISCORD_SYNCHRONIZE_VOICE_CHANNEL_ID !== channel.id) return;
